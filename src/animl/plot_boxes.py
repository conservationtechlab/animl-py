...
def plot_all_bounding_boxes(data_frame, output_dir, min_conf=0, prediction=False):
    """
    ...
    data_frame['FilePath'] = data_frame['FilePath'].astype(str)
    data_frame = data_frame.sort_values(by="FilePath")
    ...
    for index, row in data_frame.iterrows():
        ...
        elif row['FilePath'] == curr_picture[0]['FilePath']:
            curr_picture.append(row)

        # All rows for the current image have been collected
        else:
            if not os.path.exists(curr_picture[0]['FilePath']):
                continue
            # Loading the image
            img = cv2.imread(curr_picture[0]['FilePath'])

            # Getting the output destination
            file_path = curr_picture[0]['FilePath']
