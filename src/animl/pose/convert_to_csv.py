...
def process_dataset(file_path: dict, dataset_name: str):
    ...
    if dataset_name == "stanford-dogs":
        for image in enumerate(dataset):
            ...
            full_image["bbox_x"] = image[1]["img_bbox"][0]
            full_image["bbox_y"] = image[1]["img_bbox"][1]
            full_image["bbox_w"] = image[1]["img_bbox"][2]
            full_image["bbox_h"] = image[1]["img_bbox"][3]
    ...
    if dataset_name == "animal-pose":
        ...
        for annotation in dataset.get("annotations", []):
            annotation["bbox_x"] = annotation["bbox"][0]
            annotation["bbox_y"] = annotation["bbox"][1]
            annotation["bbox_w"] = annotation["bbox"][2]
            annotation["bbox_h"] = annotation["bbox"][3]
...
def merge_datasets(df_list): #concatenate datasets 
     
    result_df = pd.concat(df_list)[pd.concat(df_list)['viewpoint'] != 'undefined']
    with open('/mnt/machinelearning/Viewpoint/csvs/merged-dataset.csv', 'w') as f:
