...
the example folder. Add `-s` to sort the images into species folders. Add `-v` to add copies of the images with the bounding boxes plotted.
...
See the [Reference Manual](https://conservationtechlab.github.io/animl-py/) for detailed information on functions and usage.
...
1. Assuming a file manifest of training data with species labels, first split the data into training, validation and test splits.
   This function splits each label proportionally by the given percentages, by default 0.8 training, 0.1 validation, 0.1 Test.
